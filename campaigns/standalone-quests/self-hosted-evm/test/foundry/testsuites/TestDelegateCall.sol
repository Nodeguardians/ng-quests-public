...
import "contracts/test/utils/DelegateCaller.sol";
...
abstract contract TestDelegateCall is Test {
    ...
    function setUp() public EOA {
        ...
        sevm.execute(
            Core.Input({
                ...
                data: abi.encodePacked(
                    type(DelegateCaller).creationCode,
                    abi.encode(callee)
                )
            })
        );
        ...
        assertEq(bytecode, abi.encodePacked(type(DelegateCaller).runtimeCode));
    }

    function test_DelegateCall_View_Without_Value() external EOA {
        bytes memory rdata = sevm.execute(
            Core.Input({
                caller: origin,
                to: caller,
    ...
    function test_Cannot_DelegateCall_View_Function_With_Value() external EOA {
        vm.expectRevert();
        sevm.execute(
            Core.Input({
                caller: origin,
    ...
    function test_DelegateCall_Pure_Without_Value() external EOA {
        bytes memory rdata = sevm.execute(
            Core.Input({
                caller: origin,
                to: caller,
    ...
    function test_Cannot_DelegateCall_Pure_Function_With_Value() external EOA {
        vm.expectRevert();
        sevm.execute(
            Core.Input({
                caller: origin,
    ...
    function test_Cannot_DelegateCall_Non_Payable_With_Value() external EOA {
        vm.expectRevert();
        sevm.execute(
            Core.Input({
                caller: origin,
    ...
    function test_State_Changing_DelegateCall() external EOA {
        bytes memory rdata = sevm.execute(
            Core.Input({
                caller: origin,
                to: caller,
    ...
    function test_Cannot_DelegateCall_With_More_Value_Than_Balance()
        external
        EOA
    {
    ...
    function test_Context() external {
        ...
        assertEq(rdata, abi.encode(origin, origin, caller, callee));
    }
}
