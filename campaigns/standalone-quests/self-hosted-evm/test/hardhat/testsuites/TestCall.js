...
function testCall(subsuiteName) {
  describe(subsuiteName, function () {
    ...
    before(async function () {
      ...
      CallerFactory = await ethers.getContractFactory("Caller");
    ...
    it("Should correctly deploy the Caller contract", async function () {
      callerAddress = await evm.getNextDeploymentAddress(
        "0xc0ffee254729296a45a3885639AC7E10F9d54979"
      );
      ...
      await evm.execute({
        ...
        to: ethers.constants.AddressZero,
        value: 0,
        data: CallerFactory.getDeployTransaction(calleeAddress).data,
      });
    ...
    it("Should be able to call a view function without value", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
    ...
    it("Should not be able to call a view function with value", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
        CalleeFactory.interface.encodeFunctionData("callView"),
      ]);
    ...
    it("Should be able to call a pure function without value", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
    ...
    it("Should not be able to call a pure function with value", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
        CalleeFactory.interface.encodeFunctionData("callPure"),
      ]);
    ...
    it("Should not be able to call a non payable function with value", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
    ...
    it("Should be able to call a state changing function", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
        CalleeFactory.interface.encodeFunctionData("call", [5]),
      ]);
      ...
      result = await evm.callStatic.execute({
        caller: "0xc0ffee254729296a45a3885639AC7E10F9d54979",
        to: calleeAddress,
        value: 0,
        data: CalleeFactory.interface.encodeFunctionData("value"),
    ...
    it("Should be able to call and forward the revert reason", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
    ...
    it("Should not be able to call with more value than balance", async function () {
      const calldata = CallerFactory.interface.encodeFunctionData("call", [
        CalleeFactory.interface.encodeFunctionData("call", [0]),
      ]);
    ...
    it("Should not allow calls not originating from the sEVM", async function () {
      await expect(
        evm.call(
          {
    ...
    it("Should not allow a call with value while in a readonly context", async function () {
