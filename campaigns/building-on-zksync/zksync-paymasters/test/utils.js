...
async function setTime(timeOfDay) {
  const provider = getProvider();

  const block = await provider.getBlock("latest");
  const currentTimeOfDay = block.timestamp % 86400;

  const targetTimeOfDay =
    Math.floor(new Date(`2024-01-01T${timeOfDay}Z`).getTime() / 1000) % 86400;

  let newTime = block.timestamp + 86400 + targetTimeOfDay - currentTimeOfDay;
...
async function advanceTime(numOfDays) {
  const provider = getProvider();

  const block = await provider.getBlock("latest");
  const newTime = block.timestamp + 86400 * numOfDays;
