...
const { Provider, Wallet } = zksync;

function getProvider() {
  const rpcUrl = hre.network.config.url;
  return new Provider(rpcUrl, undefined, {cacheTimeout: -1});
}
...
async function sendTransaction(from, to, functionSig, args, signer) {
  const provider = getProvider();
  const iface = new ethers.Interface([
    `function ${functionSig} external`
  ]);
  const data = iface.encodeFunctionData(functionSig, args);
  const txNonce = await provider.getTransactionCount(from);
  ...
  const tx = {
    ...
    type: 113,
    customData: {
      ergsPerPubdata: zksync.utils.DEFAULT_GAS_PER_PUBDATA_LIMIT
    },
    value: 0n,
  ...
  const signature = signer.signingKey.sign(signedTxHash).serialized;
  ...
  const sentTx = await provider.broadcastTransaction(
    zksync.types.Transaction.from(tx).serialized
  );
  await sentTx.wait();
}
